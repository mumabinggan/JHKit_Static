...
- (void) contentOffsetChanged:(NSDictionary*) change {
    ...
    if (_scrollView.isDragging) {
        ...
        if (invokeRatio && [_indicator respondsToSelector:@selector(pullingWithRatio:offset:)]) {
            [_indicator pullingWithRatio:ratio offset:-offsetY];
        }
        else if (invokeRatio && [_indicator respondsToSelector:@selector(pullingWithRatio:)]) {
...
- (void) contentOffsetChanged:(NSDictionary*) change {
    ...
    if (_scrollView.isDragging) {
        ...
        if (invokeRatio && [_indicator respondsToSelector:@selector(pullingWithRatio:offset:)]) {
            [_indicator pullingWithRatio:ratio offset:offsetY - beginY];
        }
        else if (invokeRatio && [_indicator respondsToSelector:@selector(pullingWithRatio:)]) {
            [_indicator pullingWithRatio:ratio];
        }
